...
void RDA5807_Write()
{
  ...
  for ( int i=2;i<7;i++){ Wire_write16(RDA5807_Reg[i]);}
...
void RDA5807_Reset()
{
  for(byte i=0;i<7;i++){RDA5807_Reg[i]=RDA5807_Default[i];}
...
{
  if (volNew>15) {volAct=15;return; }
  if (volNew<0)  {volAct=0; return; }
  volAct=volNew;
...
void RDA5807_setFreq(float freqNew)
{
  ...
  Wire.beginTransmission(RDA5807SEQ);
  Wire_write16(RDA5807_Reg[2]);      //   0xD009);
...
void RDA5807_RDS()
{
  char curFlag=' ';
    ...
    if((rdsData[1] & 0xF000)==0x2000){ 
      //Textflag auswerten, um ggf. die Zeile zu leeren
